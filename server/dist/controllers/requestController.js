...
class requestController {
  ...
  static getAll(req, res) {
    ...
    clientPool.connect().then(client => {
      return client.query(queryObject).then(requests => {
        client.release();
  ...
  static getAllFilter(req, res) {
    ...
    clientPool.connect().then(client => {
      return client.query(filter.fullQueryObject).then(requests => {
        client.release();
  ...
  static addRequest(req, res) {
    ...
    clientPool.connect().then(client1 => {
      return client1.query({ text: 'SELECT description FROM requests WHERE description=$1 AND status=$2', values: [req.body.description, 'pending'] }).then(requests => {
        client1.release();
        if (!requests.rows[0]) {
          clientPool.connect().then(client => {
            return client.query({ text: 'INSERT INTO Requests(title,description, category, image, status, dated, userid) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
              values: [req.body.title, req.body.description, req.body.category, req.body.image, 'pending', dateToday.toLocaleDateString(), req.decoded.id]
            }).then(results => {
              client.release();
  ...
  static modifyRequest(req, res) {
    ...
    clientPool.connect().then(client => {
      ...
      return client.query({ text: 'UPDATE Requests SET title=$1,description=$2, category=$3, image=$4 WHERE id=$5 RETURNING *',
        values: [selectedRequest.title, selectedRequest.description, selectedRequest.category, selectedRequest.image, parseInt(req.params.requestId, 10)]
      }).then(result => {
        client.release();
  ...
  static deleteRequest(req, res) {
    clientPool.connect().then(client => {
      ...
      return client.query(queryObject).then(response => {
