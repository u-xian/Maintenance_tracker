...
class userController {
  ...
  static signUp(req, res) {
    const hashedPassword = _bcryptjs2.default.hashSync(req.body.password, 8);
    clientPool.connect().then(client => {
      return client.query({ text: 'SELECT email FROM Users WHERE email=$1', values: [req.body.email] }).then(requests => {
        client.release();
        if (requests.rows[0]) {
          if (requests.rows[0].email === req.body.email) return res.status(200).json({ message: 'User already exists' });
        }
        clientPool.connect().then(client2 => {
          return client2.query({ text: 'INSERT INTO users (firstname, lastname,email,password, role) VALUES ($1, $2, $3, $4, $5) RETURNING id, email, firstname, lastname, role',
            ...
            const token = _jsonwebtoken2.default.sign({ id, email, role }, process.env.secret_key, { expiresIn: '24h' });
  ...
  static login(req, res) {
    clientPool.connect().then(client => {
      return client.query({ text: 'SELECT * FROM users WHERE email=$1', values: [req.body.email] }).then(result => {
        client.release();
        if (!result.rows[0]) return res.status(401).json({ message: 'No account with this email address' });
        _bcryptjs2.default.compare(req.body.password, result.rows[0].password).then(response => {
  ...
  static resetPassword(req, res) {
    const hashedPassword = _bcryptjs2.default.hashSync(req.body.password, 8);
    clientPool.connect().then(client => {
      return client.query({ text: 'UPDATE users SET password=$1 WHERE id=$2 AND firstname =$3  RETURNING *',
