...
var _requestController = require('./requestController');
...
class adminController {
  ...
  static getAll(req, res) {
    ...
    if (req.query.status || req.query.category || req.query.dated) {
      _requestController2.default.getAllFilter(req, res);
      return;
    }
    if (req.query.page) {
  ...
  static modifyStatus(req, res) {
    clientPool.connect().then(client => {
      return client.query({ text: 'SELECT * FROM requests WHERE id=$1', values: [parseInt(req.params.requestId, 10)] }).then(request => {
  ...
  static makeAdmin(req, res) {
    clientPool.connect().then(client => {
      return client.query({ text: 'SELECT * FROM users WHERE id=$1', values: [parseInt(req.params.userId, 10)] }).then(users => {
